...
//   - msys2:      C:\msys64\usr\bin\bash.exe -lc <cmd>  (POSIX; MSYSTEM=MINGW64;
//                 full GCC/mingw64 toolchain). NOT msys2.exe: that is the
//                 console-allocating Cygwin launcher, which returns exit 0 with
//                 zero bytes on both streams under piped stdio.
...
/**
 * Build the child environment: DSH_* over the process environment. For WSL,
 * only variables listed in WSLENV cross the boundary, so every DSH_* key is
 * added to the allow-list (WSLENV is a : separated VAR[/flag] list).
 *
 * WSLENV normally already exists for reasons unrelated to this plugin (Windows
 * Terminal exports something like "WT_SESSION:WT_PROFILE_ID:"), so the DSH_*
 * keys are LAYERED onto the inherited allow-list instead of replacing it -
 * rebuilding it would silently cut Windows Terminal's session/profile
 * variables off from every WSL call. An explicit WSLENV supplied by the caller
...
export function buildEnv(shell, dshEnv, inheritedWslenv = process.env.WSLENV) {
  ...
  if (shell === "wsl") {
    ...
    if (keys.length > 0) {
      ...
      // Split, don't concatenate: the inherited value typically ends with a
      // trailing ":" (Windows Terminal's form), so string-joining would emit a
      // malformed empty entry ("WT_SESSION:WT_PROFILE_ID::DSH_WEB_URL"). WSLENV
      // itself is a key of dshEnv and must not be appended as a crossed var.
