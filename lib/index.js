 ...
 * key is appended there (WSLENV is a : separated VAR[/flag] list). The
 * inherited WSLENV (Windows Terminal exports one on this host) is layered
 * onto rather than replaced; pass `inheritedWslenv` to make that explicit.
 */
export function buildEnv(shell, dshEnv, inheritedWslenv = process.env.WSLENV) {
  ...
  if (shell === "wsl") {
    ...
    if (keys.length > 0) {
      ...
      // WSLENV is the allow-list for which variables cross into WSL, and the
      // host already has one (Windows Terminal exports "WT_SESSION:WT_PROFILE_ID:").
      // Rebuilding it from the DSH keys alone silently dropped those entries, so
      // layer onto the inherited list instead. Split/normalise rather than
      // string-concatenate: the inherited value ends with ':' and a naive join
      // yields an empty entry ("A:B::DSH_X"), which WSL flags as malformed.
