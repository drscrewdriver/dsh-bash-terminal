...
// MSYSTEM=MINGW64 injection here too, or the login shell sources /etc/profile
// with the default MSYS environment and /mingw64/bin (gcc, make) never joins
// PATH. The shell tool shares this exact helper.
    ...
    async execute(args, exec) {
      ...
      switch (args.action) {
        case "open": {
          ...
          // Shared helper, not an inline duplicate: it injects MSYSTEM=MINGW64
          // for msys2 (without it the login shell sources /etc/profile with the
          // default MSYS environment and /mingw64/bin never joins PATH) and
          // layers the DSH_* keys onto the inherited WSLENV allow-list for wsl.
          // A PTY session therefore gets exactly the environment a one-shot
          // shell command gets - no second copy to drift. The inherited WSLENV
          // is passed explicitly because DSH's spawnTerminal replaces the child
          // env wholesale, so an ambient value would otherwise be lost here.
          const env = buildEnv(shell, dshEnv, process.env.WSLENV);
