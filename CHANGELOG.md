...
- **WSL 的 `WSLENV` 允许列表不再被重建**：`buildEnv` 原先只从 `dshEnv` 组装 `WSLENV`，相当于把它整份替换——而 `WSLENV` 是「哪些变量能穿过 WSL 边界」的允许列表，本机已被 Windows Terminal 设为 `WT_SESSION:WT_PROFILE_ID:`，于是这两个条目在每次 WSL 调用里都被静默丢掉。现在 `buildEnv(shell, dshEnv, inheritedWslenv = process.env.WSLENV)` 在继承值上叠加：按 `:` 拆分并归一化（继承值以 `:` 结尾，字符串拼接会产出 `A:B::DSH_X` 这种带空条目的畸形值）、排除 `WSLENV` 键自身（否则会多列一个 `WSLENV`）、调用方显式传入的 `WSLENV` 优先于继承值。`lib/terminal.js` 不再自行合并，改为把 `process.env.WSLENV` 作为第三个参数显式传给 `buildEnv`（`shellEnv.collect` 不提供该变量），两层逻辑合并为一处，避免 DSH 键被重复列出。
- **交互式 `terminal` 工具改用共享 `buildEnv`**：`lib/terminal.js` 原先内联拼装 PTY 环境（`{ NO_COLOR, TERM, PAGER, GIT_PAGER, ...ctx.shellEnv.collect() }`），绕过了 `buildEnv` 的 `MSYSTEM=MINGW64` 注入，于是 msys2 交互会话以默认 MSYS 环境 source `/etc/profile`——`shell` 工具能用 `gcc`/`make`，`terminal` 里 `/mingw64/bin` 却不在 PATH 上。现在 PTY 环境同样走 `buildEnv`（含 `WSLENV` 继承），登录旗标不变。真实 PTY 已验证：提示符从 `MSYS` 变为 `MINGW64 ~`，`MSYSTEM=MINGW64` 与 `/mingw64/bin/gcc` 均成立。
...
- 测试：`buildArgv("msys2")` 断言 `-lc`、`buildEnv` 断言 `MSYSTEM=MINGW64` 且显式值优先、`WSLENV` 断言继承条目（`WT_SESSION`/`WT_PROFILE_ID`）保留 + DSH 键追加 + 精确结果 `WT_SESSION:WT_PROFILE_ID:DSH_WEB_URL` + 显式 `WSLENV` 优先 + gitbash 不带 `WSLENV`（继承值显式传入，不依赖环境变量）、候选列表中每个 `bash.exe` 必须排在 `msys2.exe` 之前、解析结果必须落在 `bash.exe`、apply 用例覆盖 msys2 的 argv/env/沙箱，`test/terminal.mjs` 新增真实 PTY 断言（msys2 会话必须带 `MSYSTEM=MINGW64` 且 PATH 含 `/mingw64/bin/gcc`），client 用例新增「宿主 `SHELLS` 与客户端列表 + 双语词典 + 描述」一致性漂移守卫。
